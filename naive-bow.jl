...
VOCAB_THRESHOLD = 1  # represents most common words, which are not class specific -> Increasing this constant is inefficient in terms of computational perform, however may a slight posiitive effect on the accuracy
VOCAB_SIZE = 30000     # gets most common words in order
SENTENCE_SIZE = 200 # increasing this constant, increases the accuracy value, in most cases.
TR_TS_SIZE = 25000
UNKNOWN = "<unk>"
...
# W = Knet.param(2,length(voc_latter)) # pos to 1, neg to 2
...
function train(; nepochs = 10, lr = 0.01)
    ...
    for epoch in 1:nepochs
        ...
        println((epoch = epoch, trn = accuracy(trn)))
    end
end
